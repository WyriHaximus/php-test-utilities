...
bare-install: ## Install dependencies ####
	echo "docker run --rm -i composer install"
	docker run --rm -i -v "`pwd`:`pwd`" -v "${COMPOSER_CACHE_DIR}:${COMPOSER_CONTAINER_CACHE_DIR}" -w "`pwd`" "${CONTAINER_NAME}" composer install
